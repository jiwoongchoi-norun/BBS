...
| 회원 탈퇴       | 완료 | `POST /bbs/withdraw`, `LOGIN.OK = 0` soft deactivate |
...
| 댓글 삭제       | 완료 | `POST /bbs/wdelete` soft delete                      |
...
- 목록 댓글 수 표시
  - `BBSW.OK = 1` 기준 댓글/대댓글 개수를 제목 옆 `[n]`으로 표시
- 목록 정렬
  - 조회수, 좋아요, 작성일 기준 `sort/order` query 정렬
  - sort/order 화이트리스트 검증 적용
- 내 글만 보기
  - 로그인 사용자는 `mine=1` query로 본인 작성글만 필터링
  - 검색/정렬/페이징 이동 시 내 글 필터 유지
- 페이지당 표시 개수 선택
  - 10, 20, 30, 50개 선택
...
- 게시글 상세 개선
  ...
  - 첨부파일 크기를 `B`, `KB`, `MB` 단위로 보기 좋게 표시
  - 좋아요/싫어요 버튼을 통합 PNG 기반 CSS sprite 아이콘 + 카운트 UI로 표시
  - 추천 active 상태는 별도 이미지 없이 버튼 배경, 테두리, 그림자로 표현
- 내 정보 페이지 추가
  - `GET /bbs/myinfo`
  - 아이디, 이름, 전화번호, 정보 갱신일 표시
  - 회원정보 수정 페이지로 이동 가능
- 아이디 찾기 추가
  - `GET /bbs/find-id`, `POST /bbs/find-id`
  - 이름과 이메일이 일치하는 활성 계정의 ID 표시
- 회원 탈퇴 추가
  - 내 정보 화면에서 비밀번호와 확인 문구 입력 후 탈퇴 처리
  - `LOGIN.OK = 0`으로 계정 비활성화하고 세션 종료
  - 탈퇴 계정은 로그인과 내 정보 조회에서 제외
- 글쓰기 작성자 자동 처리
  - 작성자 입력 항목 제거
...
- 작성자 권한 체크: 게시글 수정/삭제, 댓글 수정/삭제에 작성자 확인 적용
- 탈퇴 계정 차단: `LOGIN.OK = 1`인 활성 계정만 로그인과 내 정보 조회 허용
...
추가로 현재 코드의 회원가입/회원 탈퇴는 `LOGIN.PHONE`, `LOGIN.OK` 컬럼이 존재한다는 전제로 동작한다. `scripts/migration.sql`에는 두 컬럼 보강이 포함되어 있다.
...
2. 기존 DB라면 `LOGIN.PHONE`, `LOGIN.OK` 컬럼 존재 여부 확인
3. `npm run verify:app`
4. 로그인, 로그아웃, 아이디 찾기, 회원가입, 아이디 중복확인, 내 정보, 회원정보 수정, 회원 탈퇴 수동 테스트
5. 게시글 목록, 내 글만 보기, 댓글 수 표시, 검색, 정렬, 페이지당 표시 개수, 페이징 수동 테스트
