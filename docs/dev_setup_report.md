...
- Bootstrap 5 CDN과 공통 CSS를 사용한다.
- `partials/nav.ejs`는 모바일 접힘 메뉴를 제공한다.
- `list.ejs`는 검색 카드와 게시글 테이블 카드로 구성되어 있다.
- 게시글 테이블은 `table-responsive`와 최소 너비를 사용해 작은 화면에서 가로 스크롤로 처리한다.
- 작성, 수정, 상세, 로그인, 회원가입, 회원정보 수정 화면은 동일한 카드 스타일과 버튼 규칙을 사용한다.
...
- EJS 템플릿 렌더링 검사로 문법 오류를 확인한다.
- `/bbs/list`와 `/stylesheets/style.css`의 HTTP 200 응답을 확인한다.
- 모바일 화면에서는 내비게이션 접힘 메뉴와 테이블 가로 스크롤을 중점 확인한다.
