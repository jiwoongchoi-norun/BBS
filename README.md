...
아직 bcrypt 비밀번호 암호화는 실제 라우트에 적용되지 않았다. `bcrypt` 패키지는 설치되어 있고 회원가입에는 SHA-512 + salt 저장 코드가 일부 들어가 있지만, 로그인 검증은 salt를 사용하지 않는 비교 구조라 신규 가입 계정 로그인 흐름을 재점검해야 한다. 회원정보 수정은 여전히 평문 저장 가능성이 있다. SQL은 대부분 문자열 결합 방식으로 작성되어 있어 SQL Injection 위험이 남아 있다.
...
- 현재 프로젝트: `C:\BBS\BBS`
- 교수님 최종 프로젝트: `C:\BBS\BBS\BBS_pro_Ver`

확인 결과, 교수님 최종본에는 현재 프로젝트에 없는 페이징, 댓글 테이블(`BBSW`), 댓글 작성 라우트(`/bbs/wsave`), 댓글 표시 화면, SHA-512 + salt 기반 로그인 검증, `COUNT` 컬럼 기반 조회수 증가용 `/bbs/read_count` 라우트가 포함되어 있다.

현재 프로젝트는 교수님 최종본보다 `.env` 기반 DB 설정, `SESSION_SECRET` 필수화, Bootstrap partial 분리, custom CSS, 활성 글 필터링(`OK=1`), `VIEW_COUNT` 기반 상세 진입 시 조회수 증가, 로그인 필요 라우트 보호가 더 정리되어 있다.

### 교수님 최종본 대비 추가 필요 기능

| 기능            | 현재 프로젝트 | 교수님 최종본                                  | 보완 판단          |
| --------------- | ------------- | ---------------------------------------------- | ------------------ |
| 페이징          | 미구현        | 구현, `OFFSET ... FETCH NEXT`                  | 추가 필요          |
| 댓글            | 미구현        | `BBSW` 테이블, `/bbs/wsave`, 상세 화면 목록    | 추가 필요          |
| 댓글 수정/삭제  | 미구현        | 화면 링크만 있고 실제 라우트 불명확            | 선택 보완          |
| 대댓글          | 미구현        | 링크만 있고 DB/라우트 없음                     | 가산점 후보        |
| 좋아요/싫어요   | 미구현        | 화면 링크만 있고 실제 라우트 불명확            | 가산점 후보        |
| 파일 업로드     | 미구현        | 파일 input만 있고 `multipart/multer` 처리 없음 | 별도 구현 필요     |
| 파일 다운로드   | 미구현        | 미구현                                         | 별도 구현 필요     |
| 비밀번호 암호화 | 부분/불완전   | SHA-512 + salt 가입/로그인                     | bcrypt로 보완 권장 |

### 이미 구현된 기능

게시글 목록, 작성, 조회, 수정, soft delete, 검색, 로그인, 로그아웃, 회원가입, 회원정보 수정 화면/저장, 세션 처리, 조회수 표시/증가, Bootstrap 기반 UI는 현재 프로젝트에 구현되어 있다.

### 제출 준비 체크리스트

- `scripts/schema.sql`을 제출용 테이블 생성 SQL로 최신화해야 한다.
- 제출 SQL에는 `LOGIN`, `BBS`, `BBSW`, `BBS_SEQ`, `BBSW_SEQ`, 향후 파일 업로드 테이블이 포함되어야 한다.
- 게시판 모든 기능 화면 캡처가 필요하다.
- 화면 캡처 대상은 목록, 작성, 조회, 수정, 삭제 후 목록, 검색, 로그인, 로그아웃, 회원가입, 회원정보 수정, 조회수, 페이징, 댓글, 파일 업로드/다운로드이다.
- 소스코드 압축 전 `.env`, `node_modules`, `.git`, 실제 업로드 파일은 제외해야 한다.
